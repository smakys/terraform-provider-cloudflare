...
func TestAccCloudflareManagedTransforms_Migration_RequestOnly(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			acctest.MigrationTestStep(t, v4Config, "= 4.52.1", tmpDir, []statecheck.StateCheck{
...
func TestAccCloudflareManagedTransforms_Migration_Empty(t *testing.T) {
	// TODO: This test currently fails because the migration script doesn't add
	// empty lists for managed_request_headers and managed_response_headers when
	// they're not specified in v4. In v5, these attributes are required.
	// The migration script needs to be updated to handle this case.
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// Step 2: Migrate to v5 provider
			acctest.MigrationTestStep(t, v4Config, "~> 4.0", tmpDir, []statecheck.StateCheck{
				statecheck.ExpectKnownValue(resourceName, tfjsonpath.New("zone_id"), knownvalue.StringExact(zoneID)),
				statecheck.ExpectKnownValue(resourceName, tfjsonpath.New("managed_request_headers"), knownvalue.ListSizeExact(0)),
				statecheck.ExpectKnownValue(resourceName, tfjsonpath.New("managed_response_headers"), knownvalue.ListSizeExact(0)),
			}),
